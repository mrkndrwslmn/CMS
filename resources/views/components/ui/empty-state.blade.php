...
<div {{ $attributes->merge(['class' => 'flex flex-col items-center justify-center py-12 text-center']) }}>
    @if($icon)
        <div class="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-neutral-100">
            @if($icon === 'folder')
                <svg class="h-8 w-8 text-neutral-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
    ...
    @if($title)
        <h3 class="mt-4 text-lg font-medium text-neutral-900">{{ $title }}</h3>
    @endif
    
    @if($description)
        <p class="mt-2 max-w-sm text-sm text-neutral-500">{{ $description }}</p>
    @endif
