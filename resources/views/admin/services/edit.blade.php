...
<div class="px-6 py-8" x-data="serviceForm()">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-semibold text-primary-500 mb-1">Edit Service</h1>
            <p class="text-neutral-500 text-sm">Update service details and configuration</p>
        </div>
    ...
    <form action="{{ route('admin.services.update', $service) }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-neutral-200 p-6">
            ...
            <!-- Icon -->
            <div class="mt-6">
                <label for="icon" class="block text-sm font-medium text-neutral-700 mb-1">Icon</label>
                <select id="icon" name="icon"
                        class="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                    @foreach($icons as $value => $label)
                        <option value="{{ $value }}" {{ old('icon', $service->icon) === $value ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                <p class="mt-1 text-sm text-neutral-500">FontAwesome icon to display with this service</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-neutral-200 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" id="base_price" name="base_price" value="{{ old('base_price', $service->base_price) }}" required min="0" step="0.01"
                ...
                <div>
                    ...
                    <input type="number" id="estimated_duration_days" name="estimated_duration_days" value="{{ old('estimated_duration_days', $service->estimated_duration_days) }}" min="1"
                           class="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                           placeholder="e.g., 14">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-neutral-200 p-6">
            ...
            <div>
                ...
                <textarea id="requirements" name="requirements" rows="4"
                          class="w-full px-3 py-2 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                          placeholder="e.g., Brand guidelines, content for pages, logo files, etc.">{{ old('requirements', $service->requirements) }}</textarea>
...
function serviceForm() {
    // Initialize with existing data from the service
    const existingFeatures = @json($service->features ?? []);
    const existingSkills = @json($service->required_skills ?? []);
    
    return {
        features: existingFeatures.map(f => ({ value: f })),
        skills: existingSkills.map(s => ({ value: s })),
