{{-- Deliverable Item Partial for Admin --}}
<div class="flex items-center justify-between p-3 bg-white rounded-lg border border-neutral-100 hover:border-primary-200 transition-colors" 
     id="deliverable-item-{{ $document->documentID }}"
     data-task-id="{{ $document->taskID ?? '' }}"
     data-is-pending="{{ $document->is_deliverable && !$document->is_approved ? 'true' : 'false' }}">
        ...
        <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
                ...
                @if($document->is_deliverable)
                    ...
                    <span id="status-badge-{{ $document->documentID }}" class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium {{ $document->is_approved ? 'bg-success-100 text-success-700' : 'bg-warning-100 text-warning-700' }}">
    ...
    <div class="flex items-center gap-2 ml-3">
        ...
        @if($document->is_deliverable && !$document->is_approved)
            <button type="button" 
                    id="approve-btn-{{ $document->documentID }}"
                    onclick="approveDeliverable({{ $document->documentID }})"
                    class="inline-flex items-center px-2.5 py-1.5 bg-success-500 text-white text-xs font-medium rounded-lg hover:bg-success-600 transition-colors">
