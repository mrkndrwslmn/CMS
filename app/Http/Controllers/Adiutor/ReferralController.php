...
class ReferralController extends Controller
{
    ...
    public function dashboard(): View
    {
        ...
        return view('adiutor.referrals.dashboard', compact(
            'referralCode',
            'stats',
            'referrals',
            'pendingReferrals',
    ...
    public function share(): View
    {
        ...
        $shareTexts = [
            'email' => "Hi! I've been working with this amazing service and thought you'd love it too. Join using my referral code {$referralCode->code} and get 500 bonus points + a 15% discount coupon!",
        ...
        return view('adiutor.referrals.share', compact(
            'referralCode',
            'referralUrl',
            'shareTexts'
        ));
    ...
    public function history(Request $request): View
    {
        ...
        return view('adiutor.referrals.history', compact('referrals'));
    }
    ...
    public function sendInvitation(Request $request): JsonResponse
    {
        ...
        try {
            ...
            Log::info('Referral invitation sent by adiutor', [
                'referrer_id' => $user->id,
    ...
    /**
     * Display referral credits page (read-only, withdrawal via unified wallet)
     * 
     * Note: Adiutors withdraw referral credits through their unified wallet/payout system
     * at adiutor.earnings.wallet, NOT through a separate referral withdrawal flow.
     */
    public function credits(): View
    {
        ...
        $withdrawalPendingCredits = $user->referral_credits_pending ?? 0; // Credits pending withdrawal
        $withdrawnCredits = $user->referral_credits_withdrawn ?? 0;
        
        // Calculate potential pending credits from pending referrals
        // (referrals that signed up but haven't made a qualifying payment yet)
        $pendingReferrals = $user->referralsMade()
            ->where('status', 'pending')
            ->count();
        
        // Estimate potential earnings based on minimum tier (3% of ₱100,000 = ₱3,000 per referral)
        $minQualifyingAmount = config('referral.eligibility.min_qualifying_amount', 100000);
        $minTierPercentage = config('referral.reward_tiers.0.referrer_percentage', 3);
        $potentialCreditsPerReferral = ($minQualifyingAmount * $minTierPercentage) / 100;
        $potentialPendingCredits = $pendingReferrals * $potentialCreditsPerReferral;
        
        // Get credit transactions (referral-specific)
        $transactions = $user->referralCreditTransactions()
            ->orderBy('created_at', 'desc')
            ->limit(20)
            ->get();
        
        return view('adiutor.referrals.credits', compact(
            'availableCredits',
            'withdrawalPendingCredits',
            'potentialPendingCredits',
            'pendingReferrals',
            'withdrawnCredits',
            'transactions'
        ));
    ...
    /**
     * Note: Adiutors do NOT have separate referral withdrawal methods.
     * 
     * Referral credits for adiutors are combined with their work earnings in a unified wallet.
     * Withdrawals are processed through:
     * - adiutor.earnings.wallet (view combined balances)
     * - adiutor.earnings.request-form (request payout of all earnings including referral credits)
     * 
     * This differs from Clients who DO have a separate referral credit withdrawal system
     * because clients don't have work earnings/wallet.
     */
}
